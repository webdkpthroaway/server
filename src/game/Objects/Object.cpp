...
void WorldObject::SendObjectMessageToSet(WorldPacket *data, bool self, WorldObject* except)
{
    ...
    cell.Visit(p, message, *GetMap(), *this, GetMap()->GetVisibilityDistance() + GetVisibilityMod());
}
...
void WorldObject::SendMessageToSetExcept(WorldPacket *data, Player const* skipped_receiver)
{
    ...
    if (IsInWorld())
    {
        ...
        Cell::VisitWorldObjects(this, notifier, GetMap()->GetVisibilityDistance() + GetVisibilityMod());
    }
}
...
GameObject* WorldObject::SummonGameObject(uint32 entry, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 respawnTime, bool attach)
{
    ...
    if (!go->Create(map->GenerateLocalLowGuid(HIGHGUID_GAMEOBJECT), entry, map, x, y, z, ang, rotation0, rotation1, rotation2, rotation3, 0.0, 100, GO_STATE_READY))
    {
        delete go;
        return nullptr;
    }
